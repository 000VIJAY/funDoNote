...
namespace funDoNote.Controllers
{
    ...
    public class CollaboratorController : ControllerBase
    {
        ...
        private readonly FunDoNoteContext _funDoNoteContext;
        private readonly ICollaboratorBL _collaboratorBL;
        public CollaboratorController(IConfiguration config, FunDoNoteContext funDoNoteContext, ICollaboratorBL collaboratorBL)
        {
            _config = config;
        ...
        public async Task<IActionResult> RemoveCollaborator(int NoteId, int CollabId)
        {
            try
            {
                ...
                if(isTrue == true)
                {
                    return this.Ok(new { success = true, status = 200, message = $"Collaborated email {collab.CollabEmailId} Deleted successfully" });
                }
